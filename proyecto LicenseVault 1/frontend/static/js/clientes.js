// clientes.js — FIX #5 skeleton, #7 errores, #13 exportar CSV
let _clientes = [];
let _cliPage = 1;
  ...
  page.innerHTML = `
    <div class="page-header">
      ...
      <div style="display:flex;gap:.5rem">
        <button class="btn btn-export" onclick="exportarClientes()">↓ Exportar CSV</button>
    ...
    <div class="table-wrap">
      ...
      <!-- FIX #5: skeleton -->
      <div id="cliTableBody">${skeletonTable(5, 10)}</div>
    </div>`;
...
// FIX #13: exportar clientes a CSV
function exportarClientes() {
  exportCSV(_clientes, 'clientes.csv', [
    { key: 'nombre',   label: 'Nombre' },
    { key: 'email',    label: 'Email' },
    { key: 'pais',     label: 'País' },
    { key: 'telefono', label: 'Teléfono' },
    { key: 'historial_compras', label: 'Compras', format: v => Array.isArray(v) ? v.length : 0 },
  ]);
}
...
async function guardarCliente() {
  ...
  try {
    ...
    _cliPage = 1;
    renderCliTable();
  } catch(e) {} // FIX #7: error ya visible en toast via api.js
}
